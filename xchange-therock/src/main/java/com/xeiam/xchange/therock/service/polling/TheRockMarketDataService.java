...
public class TheRockMarketDataService extends BasePollingExchangeService implements PollingMarketDataService {

	private static final long PARTIAL_ORDERBOOK_SIZE = 200L;
	private final TheRock theRock;

	public TheRockMarketDataService(ExchangeSpecification exchangeSpecification) {

		super(exchangeSpecification);
		Assert.notNull(exchangeSpecification.getSslUri(), "Exchange specification URI cannot be null");
		theRock = RestProxyFactory.createProxy(TheRock.class, exchangeSpecification.getSslUri());
	}
	...
	public OrderBook getOrderBook(String tradableIdentifier, String currency, Object... args) throws ExchangeException, NotAvailableFromExchangeException,	NotYetImplementedForExchangeException, IOException {
		...
		theRock.getDepth(tradableIdentifier, currency, "text/html");

		// String krakenCurrencyPair =
		// KrakenUtils.createKrakenCurrencyPair(tradableIdentifier, currency);
		// KrakenDepthResult krakenDepthReturn =
		// theRock.getDepth(krakenCurrencyPair, count);
		// if (krakenDepthReturn.getError().length > 0) {
		// throw new
		// ExchangeException(Arrays.toString(krakenDepthReturn.getError()));
		// }
		// KrakenDepth krakenDepth =
		// krakenDepthReturn.getResult().get(krakenCurrencyPair);
		// List<LimitOrder> bids =
		// KrakenAdapters.adaptOrders(krakenDepth.getBids(), currency,
		// tradableIdentifier, "bids");
		// List<LimitOrder> asks =
		// KrakenAdapters.adaptOrders(krakenDepth.getAsks(), currency,
		// tradableIdentifier, "asks");
		...
		// };
		// bids.addAll(asks);
		// Date timeStamp = Collections.max(bids,
		// dateComparator).getTimestamp();
